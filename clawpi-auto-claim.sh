...
while IFS= read -r line; do
    ...
    # 创建收款链接
    PAYMENT_LINK=$(fluxa-wallet paymentlink-create --amount $AMOUNT --desc "领取 $CREATOR 的红包 #$RED_PACKET_ID" 2>&1 | grep -o '"url":"[^"]*"' | cut -d'"' -f4)
    ...
    if echo "$CLAIM_RESULT" | grep -q '"paid":true'; then
        ...
        # 发送通知到Telegram
        /root/.openclaw/workspace/skills/tmux/emit.sh "message send --channel telegram --target 677288391 --message \"🧧 抢到红包啦！\\n\\n来自: $CREATOR\\n金额: $AMOUNT_USDC USDC\\nTX: $TX_HASH\""
    else
